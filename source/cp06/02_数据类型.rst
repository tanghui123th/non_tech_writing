...
能够表示的数值范围为2\ :sup:`1024到2`-1023（开区间），超出这个范围的数无法表示。
...
:raw-latex:`\除了转义之外`，还有三种特殊用法。

-  :raw-latex:`\HHH`：HHH是三个八进制数（从000到377，即二进制0到255），代表一个字符
-  :raw-latex:`\xHH`：HH是两个十六进制数（这样也只能表示从0到255的数），代表一个字符
-  :raw-latex:`\uHHHH`：HHHH是四个十六进制数，代表一个字符
