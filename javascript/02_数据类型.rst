...
数组的length属性返回数组元素的个数。JavaScript
使用一个32位整数，保存数组的元素个数。这意味着，数组成员最多只有
4294967295 个（2^32 - 1）个，也就是说length属性的最大值就是 4294967295。
...
如果一个对象的所有键名都是正整数或零，并且有length属性，称为“类似数组的对象”（array-like
object）。但是类似数组的对象的length属性没有数组的可变性。且类似数组的对象没有数组的方法。

典型的“类似数组的对象”是函数的arguments对象，以及大多数 DOM
元素集，还有字符串。
